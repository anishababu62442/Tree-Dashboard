...
tree_tract %>%
  ...
  theme_minimal() +
  annotate("text", x=960000, y=130000, label="Staten\nIsland", color="gray50") +
  annotate("text", x=1045000, y=270000, label="Bronx", color="gray50") +
  annotate("text", x=974800, y=230000, label="Manhattan", color="gray50") +
  annotate("text", x=987000, y=143000, label="Brooklyn", color="gray50") +
  annotate("text", x=1059000, y=230000, label="Queens", color="gray50") +
...
# The below could also be done as follows, where .x is a place holder for the given column
tree_census %>% 
  mutate(
    across(c(root_stone, root_grate, root_other, trunk_wire, 
             trnk_light, trnk_other, brch_shoe , brch_light, 
             brch_other),
           ~as.numeric(ifelse(.x == "No", 0, 1)))
    )
...
# The below could be done within mutate also
tree_damage$problem_type <- as.factor(tree_damage$problem_type) 
tree_damage$problem_type <- recode(tree_damage$problem_type, 
